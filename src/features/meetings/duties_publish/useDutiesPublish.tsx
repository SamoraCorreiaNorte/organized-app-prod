...
const useDutiesPublish = (onClose: VoidFunction) => {
  ...
  const handleUpdateSchedules = (schedulesData: SchedWeekType[]) => {
    const newSchedules = structuredClone(schedulesData);
  ...
  const handleGetIncomingTalks = (schedulesData: SchedWeekType[]) => {
    const talks: OutgoingTalkExportScheduleType[] = [];

    const outgoingTalks = schedulesData.filter(
      (record) =>
  ...
  const handleFilterOutgoingTalks = (schedulesData: SchedWeekType[]) => {
    ...
    const result = schedulesData.map((schedule) => {
  ...
  const publishForType = async (
    type: MeetingType,
    months: string[],
    sourcesRemote: SourceWeekType[],
    schedulesRemote: SchedWeekType[]
  ) => {
    const sourcesLocalPublish = handleGetMaterials(sources, months, type);
    const schedulesLocalPublish = handleGetMaterials(schedules, months, type);
    ...
    let schedulesPublish = schedulesBasePublish;
    let talksPublish: OutgoingTalkExportScheduleType[] = undefined;

    if (type === 'weekend') {
      schedulesPublish = handleUpdateSchedules(schedulesBasePublish);

      if (isPublicTalkCoordinator) {
        schedulesPublish = handleFilterOutgoingTalks(schedulesPublish);
  ...
  const handlePublishSchedule = async () => {
    ...
    try {
      ...
      const { data: remoteData } = await refetch();

      if (
        Array.isArray(remoteData?.schedules) &&
        Array.isArray(remoteData?.sources)
      ) {
        const sourcesRemote = remoteData.sources;
        const schedulesRemote = remoteData.schedules;

        await publishForType('midweek', months, sourcesRemote, schedulesRemote);
        await publishForType('weekend', months, sourcesRemote, schedulesRemote);
...
export default useDutiesPublish;
